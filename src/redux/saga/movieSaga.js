...
function* dataMovies(action) {
  try {
    const resDataMovies = yield axios.get(
...
function* dataMoviesDetails(action) {
  ...
  try {
    const resDetailMovies = yield axios.get(
      `https://api.themoviedb.org/3/movie/${movie_id}?api_key=9c4e75bc7c12882201bbc19a846352ba&language=en-US`,
    );
    yield put({
      type: 'GET_MOVIE_DETAILS_SUCCESS',
      dataDetails: resDetailMovies,
    });
  } catch (err) {
    console.log(err);
  }
